...
import { Link2, LogOut, Settings2, Users } from "lucide-react";
...
import { EdgeFunctionError, startTeamsConnect, disconnectTeams, getMyTeamsIntegration, getTeamsHealth } from "@/lib/teamsIntegrationDb";
  ...
  return (
    <div className="grid gap-3">
      <div className="text-sm text-muted-foreground">
        Plug-n-play: esta tela ajuda a diagnosticar configuração (Microsoft Azure AD / secrets) quando a conexão não abre.
      </div>
      ...
      {healthQuery.data?.tenantId ? (
        <div className="rounded-2xl border border-[color:var(--sinaxys-border)] bg-[color:var(--sinaxys-bg)]/20 p-3 text-xs text-muted-foreground">
          <div className="font-semibold text-[color:var(--sinaxys-ink)]">Tenant ID (extraído do Client ID)</div>
          <div className="mt-1 break-words">{healthQuery.data.tenantId}</div>
          <div className="mt-2 opacity-80">
            Dica: Certifique-se de que o redirect_uri está configurado no Azure AD App Registration.
          </div>
        </div>
      ) : null}
      ...
      {healthQuery.data?.redirectUri ? (
        <div className="rounded-2xl border border-[color:var(--sinaxys-border)] bg-[color:var(--sinaxys-bg)]/20 p-3 text-xs text-muted-foreground">
          <div className="font-semibold text-[color:var(--sinaxys-ink)]">redirect_uri (Azure AD)</div>
...
export function TeamsIntegrationCard({ tenantId, userId }: { tenantId: string; userId: string }) {
  ...
  const { data: integration, isLoading } = useQuery({
    queryKey: ["teams-integration", tenantId, userId],
    queryFn: () => getMyTeamsIntegration(tenantId, userId),
  });
  ...
  const connectMutation = useMutation({
    mutationFn: async () => {
      ...
      setLastError(null);
      const { authUrl } = await startTeamsConnect("/integrations");
  ...
  const disconnectMutation = useMutation({
    ...
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ["teams-integration", tenantId, userId] });
      toast({ title: "Microsoft Teams desconectado" });
    },
    onError: (e: any) => {
      const err = e as EdgeFunctionError;
      const status = err?.status;
    ...
    <>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Conecte para importar reuniões gravadas do Teams para suas trilhas de aprendizado.
              </p>
            </div>
          </div>
      ...
      <Dialog open={cfgOpen} onOpenChange={setCfgOpen}>
        <DialogContent className="max-w-[92vw] rounded-3xl sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>Configuração do Microsoft Teams</DialogTitle>
          </DialogHeader>

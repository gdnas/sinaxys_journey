...
export default function TrackEdit() {
  ...
  // permission: ADMIN can edit any, HEAD can edit if same department, COLABORADOR can edit only if created_by_user_id
  ...
  const moveModule = async (id: string, dir: -1 | 1) => {
    ...
    // swap order indexes
    await upsertModule({ ...a, trackId: trackId!, orderIndex: b.orderIndex });
    await upsertModule({ ...b, trackId: trackId!, orderIndex: a.orderIndex });
    await qc.invalidateQueries({ queryKey: ["modules", trackId] });
  };
  ...
  if (!canEdit) {
    return (
      <div className="grid gap-6">
        <div className="rounded-3xl border bg-white p-6">
          <div className="text-sm font-semibold text-[color:var(--sinaxys-ink)]">Sem permissão</div>
          <p className="mt-2 text-sm text-muted-foreground">Você não tem permissão para editar esta trilha.</p>
          <Button asChild variant="outline" className="mt-4 rounded-xl">

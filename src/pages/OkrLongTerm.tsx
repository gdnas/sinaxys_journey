...
export default function OkrLongTerm() {
    ...
    const grouped = useMemo(() => {
        ...
        return order.filter(k => (m.get(k) ?? []).length).map(k => ({
            horizon: k,
            list: m.get(k) ?? []
        }));
    }, [items]);
            ...
            <Card className="rounded-3xl border-[color:var(--sinaxys-border)] bg-white p-6">
                <div
                    ...
                    {isAdminish ? (<Button
                        ...
                        <Plus className="mr-2 h-4 w-4" />Novo objetivo de longo prazo
                                    </Button>) : null}
                </div>
                <Separator className="my-5" />
                {items.length ? (<div className="grid gap-6">
                    {grouped.map(g => (<div key={g.horizon} className="grid gap-3">
                            ...
                            {g.list.map(it => (<div
                                ...
                                <div className="min-w-0">
                                    <div className="flex flex-wrap items-center gap-2">
                                        <span
                                            className="rounded-full bg-[color:var(--sinaxys-tint)] px-3 py-1 text-[11px] font-semibold text-[color:var(--sinaxys-ink)] ring-1 ring-[color:var(--sinaxys-border)]">
                                            {horizonLabel(it.horizon_years)}
                                        </span>
                                        {it.target_year ? (<span
                                            className="rounded-full bg-white px-3 py-1 text-[11px] font-semibold text-[color:var(--sinaxys-ink)] ring-1 ring-[color:var(--sinaxys-border)]">Alvo: {it.target_year}
                                        </span>) : null}
                                    </div>
                                    <div
                                        className="mt-2 line-clamp-2 text-sm font-semibold text-[color:var(--sinaxys-ink)]">{it.title}</div>
                                    {it.description ? <div className="mt-1 line-clamp-2 text-sm text-muted-foreground">{it.description}</div> : null}
                                </div>
                                {isAdminish ? (<div className="flex items-center justify-end gap-2 md:pt-1">
                                    <Button
                                        variant="outline"
                                        size="icon"
                                    ...
                                    <Button
                                        variant="outline"
                                        size="icon"
                                        className={"h-10 w-10 rounded-xl border-destructive/40 bg-destructive/5 text-destructive " + "hover:bg-destructive/10 hover:text-destructive"}
                                        onClick={() => {
                                            setDelId(it.id);
                                            setDelOpen(true);
                                        }}
                    ...
                    className="rounded-2xl bg-[color:var(--sinaxys-bg)] p-4 text-sm text-muted-foreground">Nenhum objetivo de longo prazo ainda. Comece com 3–5 objetivos estratégicos bem claros.
                              </div>)}
            </Card>
            <Dialog
                open={open}
                    ...
                    <div className="grid gap-4">
                        ...
                        <div className="grid gap-2">
                            ...
                            <Input
                                ...
                                placeholder="Ex.: Ser o principal..." />
                        </div>
                        <div className="grid gap-2">

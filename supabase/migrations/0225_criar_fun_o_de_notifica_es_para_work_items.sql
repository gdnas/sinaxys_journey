...
-- 1. Função para criar notificações de work_items (similar a log_work_item_event)
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    IF NEW.assignee_user_id IS NOT NULL AND NEW.assignee_user_id <> actor THEN
      ...
      EXCEPTION WHEN OTHERS THEN
        -- Se falhar, não propagar erro para não quebrar insert principal
        NULL;
      END;
    END IF;
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    -- EVENTO 5 (opcional): Mudança de status → notificar assignee ou criador
